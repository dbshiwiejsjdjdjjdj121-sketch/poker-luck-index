...
const data={festivals,destinations,sources};
const now=new Date("2026-09-07T12:00:00Z");
const sample=structuredClone(festivals.find(f=>f.id==="virginia-2026")!);
test("seed contains at least 20 distinct validated overviews",()=>{assert.ok(festivals.length>=20);assert.deepEqual(validateGuide(data),[])});
